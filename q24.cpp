...
// Function to simulate the 3-stage pipeline and calculate total cycles
int simulatePipeline(int numInstructions) {
    int fetch_cycles = 1;   // Fetch stage takes 1 cycle
    int decode_cycles = 1;  // Decode stage takes 1 cycle
    int execute_cycles = 1; // Execute stage takes 1 cycle

    int total_cycles = 0;

    // Calculate total cycles considering pipeline stages
